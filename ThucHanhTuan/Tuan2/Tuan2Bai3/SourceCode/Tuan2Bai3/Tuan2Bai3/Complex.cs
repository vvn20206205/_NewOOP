...
internal class Complex {
    ...
    public void HandleString(string iInputString) {
        ...
        int _ViTriDauCong = iInputString.LastIndexOf("+");
        int _ViTriDauTru = iInputString.LastIndexOf("-");
        int _ViTriPhanAoI = iInputString.LastIndexOf("i");
        if(_ViTriPhanAoI<0) {
            Real=Convert.ToSingle(iInputString.Substring(0));
            Imaginary=0;
        } else {
            if(_ViTriDauCong>0) {
                Real=Convert.ToSingle(iInputString.Substring(0,_ViTriDauCong));
                Imaginary=Convert.ToSingle(iInputString.Substring(_ViTriDauCong,_ViTriPhanAoI-_ViTriDauCong));
            } else {
                if(_ViTriDauTru>0) {
                    Real=Convert.ToSingle(iInputString.Substring(0,_ViTriDauTru));
                    Imaginary=Convert.ToSingle(iInputString.Substring(_ViTriDauTru,_ViTriPhanAoI-_ViTriDauTru));
                } else {
                    Real=0;
                    Imaginary=Convert.ToSingle(iInputString.Substring(0,_ViTriPhanAoI));
                }
            }
        }
    }
